...
interface SplashCursorProps {
  ...
  BACK_COLOR?: ColorRGB;
  TRANSPARENT?: boolean;
  colorMode?: 'white' | 'blue'; // Added for monochromatic control
}
...
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    // MODIFIED: Monochromatic color generation with reduced intensity
    function generateColor(): ColorRGB {
      if (colorMode === 'white') {
        // Pure white with subtle variations
        const intensity = 0.3 + Math.random() * 0.2; // Reduced from 0.8-1.0 to 0.3-0.5
        return { r: intensity, g: intensity, b: intensity };
      } else {
        // Blue theme matching #3B82F6 with reduced intensity
        const baseBlue = 0.96;
        const intensity = 0.3 + Math.random() * 0.2; // Reduced from 0.7-1.0 to 0.3-0.5
        return {
          r: 0.23 * intensity,
          g: 0.51 * intensity,
          b: baseBlue * intensity
        };
      }
    }
